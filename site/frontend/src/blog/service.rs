...
impl BlogService {
    ...
    pub async fn get_post_by_id(&self, post_id: &str) -> Result<Option<BlogPost>, String> {
        self.repository.get_post_by_id(post_id).await
    }

    pub async fn get_post_by_slug(&self, slug: &str) -> Result<Option<BlogPost>, String> {
        self.repository.get_post_by_slug(slug).await
    }

    pub async fn list_published_posts(&self, limit: i32) -> Result<Vec<BlogPostSummary>, String> {
